...
# ðŸŽ¯ PLAN DE IMPLEMENTACIÃ“N PASO A PASO
...
#### **DÃA 1 - PARTE 1: MÃ©tricas Base** (3-4 horas)

##### **Step 1.1: Agregar Tiempo de ConfirmaciÃ³n al Schema**
```bash
...
export const StoreMetricSchema = z.object({
  ...
  averageConfirmationTime: z.number().optional(), // â† NUEVO (en minutos)
...
// Agregar nuevo schema para distribuciÃ³n horaria
...
// Agregar nuevo schema para distribuciÃ³n semanal
...
##### **Step 1.2: Calcular MÃ©tricas en getMetricsFlow.ts**
```bash
Archivo: src/ai/flows/getMetricsFlow.ts
```

**AcciÃ³n 1 - Tiempo de confirmaciÃ³n por tienda**:
```typescript
// En el loop de confirmedOrders (lÃ­nea ~190)
...
// Al calcular aggregatedStoreMetrics (lÃ­nea ~420)
const aggregatedStoreMetrics = Object.entries(storeData).map(([name, data]) => {
  // Calcular promedio de tiempo de confirmaciÃ³n
  ...
  return {
    ...
    averageConfirmationTime: avgConfirmationTime, // â† NUEVO
    topProducts,
    dailyOrderVariation,
...
**AcciÃ³n 2 - DistribuciÃ³n horaria**:
```typescript
// DespuÃ©s del loop de allOrders (lÃ­nea ~185)
...
allOrders.forEach((order) => {
  if (order.createdAt && typeof order.createdAt.toDate === 'function') {
    ...
    const dayOfWeek = orderDate.getDay(); // 0 = Domingo, 6 = SÃ¡bado
...
// Al final del return (lÃ­nea ~630)
...
#### **DÃA 1 - PARTE 2: Dashboard Principal** (2-3 horas)

##### **Step 1.3: Agregar KPI de Tiempo de ConfirmaciÃ³n**
```bash
...
**AcciÃ³n**:
```typescript
// En la secciÃ³n de KPIs (lÃ­nea ~200)
...
<Card>
  <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
    <CardTitle className="text-sm font-medium">Tiempo Prom. ConfirmaciÃ³n</CardTitle>
...
##### **Step 1.4: Agregar GrÃ¡fico de DistribuciÃ³n Horaria**
```typescript
// DespuÃ©s del grÃ¡fico de Top Provincias (lÃ­nea ~850)
<Card className="col-span-2">
  <CardHeader>
    <CardTitle className="flex items-center gap-2">
      <Clock className="w-5 h-5" />
      DistribuciÃ³n Horaria de Pedidos
    </CardTitle>
    <CardDescription>Pedidos por hora del dÃ­a</CardDescription>
  </CardHeader>
  <CardContent>
...
##### **Step 1.5: Agregar GrÃ¡fico de DistribuciÃ³n Semanal**
```typescript
// DespuÃ©s del grÃ¡fico horario
<Card className="col-span-2">
  <CardHeader>
    <CardTitle className="flex items-center gap-2">
...
**Total DÃ­a 1**: 4-5 horas

---

### **DÃA 2: ANÃLISIS DE ENTREGAS** (4-5 horas)
...
**AcciÃ³n**:
```typescript
// En el loop de deliveredOrders (lÃ­nea ~240)
...
export const CourierMetricSchema = z.object({
  ...
  averageDeliveryTime: z.number().optional(), // â† NUEVO (horas)
  minDeliveryTime: z.number().optional(),     // â† NUEVO
  maxDeliveryTime: z.number().optional(),     // â† NUEVO
  stdDevDeliveryTime: z.number().optional(),  // â† NUEVO
});

// Al calcular aggregatedCourierMetrics (lÃ­nea ~380)
...
#### **Step 2.2: Agregar GrÃ¡fico Box Plot de Tiempos de Entrega**
```bash
...
**AcciÃ³n**:
```typescript
// Agregar despuÃ©s del grÃ¡fico de couriers (lÃ­nea ~400)
<Card className="col-span-2">
  <CardHeader>
    <CardTitle className="flex items-center gap-2">
              ...
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="bg-background border rounded p-3 shadow-md">
                    ...
                    <p>MÃ­nimo: {data.minDeliveryTime?.toFixed(1)} hrs</p>
                    <p>MÃ¡ximo: {data.maxDeliveryTime?.toFixed(1)} hrs</p>
                    <p>Desv. Est.: {data.stdDevDeliveryTime?.toFixed(1)} hrs</p>
                    <p className="text-xs text-muted-foreground mt-2">
                      {data.stdDevDeliveryTime < 12 ? 'âœ… Consistente' : 'âš ï¸ Variable'}
                    </p>
                  </div>
                );
              }
...
**Total DÃ­a 2**: 4-5 horas

---

### **DÃA 3: STOCK CRÃTICO** (4-5 horas)

#### **Step 3.1: Crear Vista de Stock CrÃ­tico**
```bash
Archivo: src/app/(app)/dashboard/inventory-status/page.tsx
```

**AcciÃ³n - Agregar secciÃ³n de alertas**:
```typescript
...
// Agregar antes de la tabla de previsiÃ³n
<div className="grid gap-4 md:grid-cols-3 mb-6">
  <Card className="border-red-500">
    <CardHeader className="pb-3">
      <CardTitle className="text-sm font-medium flex items-center gap-2">
        <AlertCircle className="w-4 h-4 text-red-500" />
        Stock CrÃ­tico (&lt; 7 dÃ­as)
      </CardTitle>
    </CardHeader>
    <CardContent>
...
#### **Step 3.2: Agregar GrÃ¡fico de Gauge para Stock**
```typescript
// Agregar despuÃ©s de las cards
<Card className="col-span-2">
  <CardHeader>
    <CardTitle>DistribuciÃ³n de Stock por Urgencia</CardTitle>
  </CardHeader>
  <CardContent>
    <ChartContainer config={{}} className="h-[300px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie
            data={[
              { name: 'CrÃ­tico', value: criticalStock.length, fill: '#ef4444' },
...
**Total DÃ­a 3**: 4-5 horas

---

### **DÃA 4-5: NUEVA PÃGINA /dashboard/products** (8-10 horas)

#### **Step 4.1: Calcular MÃ©tricas de Productos**
```bash
Archivo: src/ai/flows/getMetricsFlow.ts
```

**AcciÃ³n - Agregar anÃ¡lisis de rentabilidad**:
```typescript
// DespuÃ©s del cÃ¡lculo de purchasedProductData (lÃ­nea ~200)
...
#### **Step 4.2: Crear pÃ¡gina /dashboard/products**
```bash
Archivo: src/app/(app)/dashboard/products/page.tsx
```

**Estructura completa de la pÃ¡gina** (ver archivo adjunto)

**Tiempo estimado**: 6-8 horas

---

**Total DÃ­as 4-5**: 8-10 horas
...
1. âœ… Agregar `averageConfirmationTime` a StoreMetricSchema
2. âœ… Crear HourlyDistributionSchema
3. âœ… Crear WeekdayDistributionSchema
4. âœ… Agregar campos de delivery a CourierMetricSchema
5. âœ… Crear ProductProfitabilitySchema
6. âœ… Actualizar GetMetricsOutputSchema

### **Flow** (`src/ai/flows/getMetricsFlow.ts`):
1. âœ… Calcular tiempo de confirmaciÃ³n
2. âœ… Calcular distribuciÃ³n horaria
3. âœ… Calcular distribuciÃ³n semanal
4. âœ… Calcular tiempos de entrega por courier
5. âœ… Calcular rentabilidad de productos

### **Dashboards**:
1. âœ… `src/app/(app)/dashboard/page.tsx` - Agregar 3 grÃ¡ficos nuevos
2. âœ… `src/app/(app)/dashboard/shipments/page.tsx` - Agregar 1 grÃ¡fico
3. âœ… `src/app/(app)/dashboard/inventory-status/page.tsx` - Agregar alertas y 1 grÃ¡fico
4. âœ… `src/app/(app)/dashboard/products/page.tsx` - CREAR NUEVA PÃGINA

---

## â±ï¸ ESTIMACIÃ“N TOTAL

- **DÃ­a 1**: 4-5 horas (Fundamentos + Dashboard principal)
- **DÃ­a 2**: 4-5 horas (AnÃ¡lisis de entregas)
- **DÃ­a 3**: 4-5 horas (Stock crÃ­tico)
- **DÃ­as 4-5**: 8-10 horas (Nueva pÃ¡gina productos)

**TOTAL**: 20-25 horas (2.5-3 dÃ­as de trabajo efectivo)

---

## ðŸŽ¯ ORDEN DE EJECUCIÃ“N RECOMENDADO

1. âœ… Schemas (completar primero)
2. âœ… CÃ¡lculos en getMetricsFlow.ts (uno por uno)
3. âœ… Dashboard principal (validar datos)
4. âœ… Shipments (agregar tiempo entrega)
5. âœ… Inventory Status (alertas stock)
6. âœ… Nueva pÃ¡gina Products (al final)
...
- PÃ¡gina /dashboard/financials
- AnÃ¡lisis ABC de productos
- Tasa de conversiÃ³n por tienda

**FASE 3** (Semana 3):
- PÃ¡gina /dashboard/customers
- Cohort analysis
...
**Â¿Comenzamos con el DÃ­a 1?** ðŸš€
