...
interface AdvisorPerformance extends PerformanceMetrics, ActivityMetrics {
  id: string;
  name: string;
  callTarget?: number;
  compliance?: number;
...
interface DailyPerformanceData {
  [agentId: string]: {
    [date: string]: PerformanceMetrics &
      ActivityMetrics & {
        callTarget?: number;
        compliance?: number;
        scheduledHours?: number;
...
const calculateActualHoursFromTimes = (
...
const computeCompliancePercentage = (
  actualHours: number,
  scheduledHours: number
): number | undefined => {
...
export default function AdvisorPerformancePage() {
  ...
  const showDailyBreakdown = useMemo(
    () =>
      date?.from && date.to
        ? differenceInCalendarDays(date.to, date.from) >= 0
        : false,
    [date]
  );
      ...
      const scheduleEntries = await Promise.all(
        agentIds.map(async (id) => {
          try {
            ...
            console.warn(
              `[SCHEDULE] No se pudo cargar horario para ${id}, usando por defecto.`,
              error
            );
  ...
  const getScheduleForAgent = useCallback(
    (agentId: string): Schedule => {
      return schedules[agentId] ?? defaultSchedule;
    },
    [schedules]
  );

  const calculateScheduledHoursForRange = useCallback(
    (agentId: string, from: Date, to: Date) => {
      ...
      return intervalDays.reduce(
        (total, day) => total + calculateHoursForDay(schedule, day),
        0
      );
    },
    [getScheduleForAgent]
  ...
  const checkAndBackfillMissingDataInBackground = useCallback(
    async (startDate: Date, endDate: Date): Promise<void> => {
      try {
        // ⚠️ PREVENIR MÚLTIPLES EJECUCIONES
        if (isBackfillInProgress) {
          console.log(
            "[PERFORMANCE] 🔄 Backfill ya está en progreso, saltando nueva verificación"
          );
          return;
        }
        ...
        console.log(
          "[PERFORMANCE] 🔧 Datos faltantes detectados en background:",
          missingDays
        );
        ...
        console.error(
          "[PERFORMANCE] ❌ Error verificando datos faltantes en background:",
          error
        );
  ...
  const processCallsData = useCallback(
    (calls: any[]) => {
      ...
      const ensureAgent = (id: string, displayName?: string) => {
        const fallbackName =
          displayName ||
          (agentMap as any)[id] ||
          (id && id !== "unknown" ? `Ext ${id}` : "Sin asignar");
      ...
      Object.keys(agentMap).forEach((id) =>
        ensureAgent(id, (agentMap as any)[id])
      );
      ...
      calls.forEach((call: any) => {
        ...
        const agentDisplayName =
          call.agentName ||
          (agentMap as any)[agentId] ||
          (agentId === "unknown" ? "Sin asignar" : `Ext ${agentId}`);

        ensureAgent(agentId, agentDisplayName);
        performanceByAgent[agentId].name = agentDisplayName;

        const isEffective =
          call.disposition === "answered" && (call.seconds || 0) > 0;
        const callDate =
          call.callDate ||
          call.callstart?.substring(0, 10) ||
        ...
        if (
          !performanceByAgent[agentId].firstCallTime ||
          callTime < performanceByAgent[agentId].firstCallTime!
        ) {
          performanceByAgent[agentId].firstCallTime = callTime;
        }
        if (
          !performanceByAgent[agentId].lastCallTime ||
          callTime > performanceByAgent[agentId].lastCallTime!
        ) {
      ...
      Object.values(performanceByAgent).forEach((agent) => {
        agent.effectivenessRate =
          agent.totalCalls > 0
            ? (agent.effectiveCalls / agent.totalCalls) * 100
            : 0;
        agent.averageCallDuration =
          agent.effectiveCalls > 0
      ...
      Object.keys(dailyPerformance).forEach((agentId) => {
        ...
        Object.entries(dailyPerformance[agentId]).forEach(([dayKey, stats]) => {
          stats.effectivenessRate =
            stats.totalCalls > 0
              ? (stats.effectiveCalls / stats.totalCalls) * 100
              : 0;
          stats.averageCallDuration =
            stats.effectiveCalls > 0
              ? stats.totalSeconds / stats.effectiveCalls
              : 0;

          const scheduleDate = new Date(`${dayKey}T00:00:00`);
          const scheduledHours = calculateHoursForDay(
            agentSchedule,
            scheduleDate
          );
          const actualHours = calculateActualHoursFromTimes(
            stats.firstCallTime,
            stats.lastCallTime
          );
          ...
          stats.compliance = computeCompliancePercentage(
            actualHours,
            scheduledHours
          );
        ...
        const scheduledHoursInRange = calculateScheduledHoursForRange(
          agentId,
          rangeStart,
          rangeEnd
        );
        ...
        agentStats.compliance = computeCompliancePercentage(
          accumulatedActualHours,
  ...
  const checkAndBackfillMissingData = useCallback(
    async (startDate: Date, endDate: Date): Promise<boolean> => {
      try {
        ...
        console.log(
          "[PERFORMANCE] 🔧 Datos faltantes detectados:",
          missingDays
        );
        ...
        console.error(
          "[PERFORMANCE] ❌ Error verificando datos faltantes:",
          error
        );
        toast({
  ...
  const handleBackfillComplete = useCallback(() => {
    ...
    toast({
      ...
      description:
        "Los datos están ahora disponibles y se han actualizado automáticamente.",
      variant: "default",
    });
  ...
  const handleBackfillError = useCallback(
    (error: string) => {
      setShowBackfillProgress(false);
      setBackfillDates(null);
  ...
  const fetchAndProcessData = useCallback(
    async (silent = false, isAutoRefresh = false) => {
      if (!date?.from) return;
      ...
      try {
        ...
        const isViewingToday =
          format(date.from, "yyyy-MM-dd") === today &&
        ...
        if (isViewingToday) {
          console.log(
            "[PERFORMANCE] 🔥 DÍA DE HOY - mostrando Firestore y actualizando en background..."
          );
          ...
          console.log(
            `[PERFORMANCE] ⚡ Cargando Firestore para mostrar inmediatamente...`
          );
          const firestoreResponse = await fetch(
            `/api/zadarma/calls?${firestoreParams.toString()}`
          );
          const firestoreData = await firestoreResponse.json();
          calls = firestoreData.calls || [];
          console.log(
            `[PERFORMANCE] ✅ Firestore cargado: ${calls.length} llamadas (mostrando AHORA)`
          );
          ...
          if (isAutoRefresh) {
            setTimeout(async () => {
              console.log(
                "[PERFORMANCE] 🔄 Actualizando desde API Zadarma (auto-refresh)..."
              );
              ...
              try {
                const statsResponse = await fetch(
                  `/api/zadarma/stats?${refreshParams.toString()}`
                );
                const statsData = await statsResponse.json();

                if (
                  statsData.status === "success" ||
                  statsData.status === "partial"
                ) {
                  const freshCalls = statsData.stats || [];
                  const saved =
                    typeof statsData.savedTotal === "number"
                      ? statsData.savedTotal
                      : statsData.count ?? freshCalls.length;
                  console.log(
                    `[PERFORMANCE] ✅ Background API completada - ${freshCalls.length} llamadas, guardadas: ${saved}`
                  );

                  if (
                    Array.isArray(statsData.lockedDays) &&
                    statsData.lockedDays.length > 0
                  ) {
                    console.log(
                      "[PERFORMANCE] ⏳ Sync bloqueada para:",
                      statsData.lockedDays.join(", ")
                    );
                  }
                  if (
                    Array.isArray(statsData.errors) &&
                    statsData.errors.length > 0
                  ) {
                    console.warn(
                      "[PERFORMANCE] ⚠️ Problemas en background:",
                      statsData.errors
                    );
                  }
                  ...
                  console.log(
                    "[PERFORMANCE] 🎯 UI actualizada con datos frescos de API"
                  );
                } else {
                  console.warn(
                    "[PERFORMANCE] ⚠️ Background API respondió con estado:",
                    statsData.status,
                    statsData.message
                  );
                }
                ...
                console.warn(
                  "[PERFORMANCE] ⚠️ Error en background update:",
                  error
                );
              }
            }, 100);
            ...
            console.log(
              "[PERFORMANCE] 🚫 NO es auto-refresh - omitiendo actualización API background"
            );
          }
        } else {
          ...
          console.log(
            `[PERFORMANCE] 📊 Consultando Firestore: ${format(
              date.from,
              "yyyy-MM-dd"
          ...
          calls = data.calls || [];
          console.log(
            "[PERFORMANCE] Datos recibidos desde Firestore:",
            calls.length,
            "llamadas"
          );
        }
        ...
        if (!isAutoRefresh && !silent && date.from && !isViewingToday) {
          ...
          setTimeout(async () => {
            if (date.from) {
              await checkAndBackfillMissingDataInBackground(
                date.from,
                date.to || date.from
              );
            }
  ...
  useEffect(() => {
    ...
    const refreshInterval = setInterval(() => {
      ...
      if (!isBackfillInProgress) {
        ...
        console.log(
          "[AUTO-REFRESH] ⏸️  Saltando refresh - backfill en progreso"
        );
      }
    }, 60000);
  ...
  const handleForceSyncSelection = useCallback(async () => {
    ...
    const rangeLabel =
      differenceInCalendarDays(rangeEnd, rangeStart) === 0
        ? format(rangeStart, "dd LLL, y", { locale: es })
        : `${format(rangeStart, "dd LLL, y", { locale: es })} → ${format(
            rangeEnd,
            "dd LLL, y",
            { locale: es }
          )}`;
    ...
    try {
      ...
      const totalGuardadas =
        typeof data.savedTotal === "number"
          ? data.savedTotal
          : Object.values(data.savedPerDay || {}).reduce(
      ...
      const hadLocks =
        Array.isArray(data.lockedDays) && data.lockedDays.length > 0;
      const hadErrors = Array.isArray(data.errors) && data.errors.length > 0;
      const errorSummary = hadErrors
        ? data.errors
            .map((entry: any) => `${entry.date}: ${entry.message}`)
            .join(" | ")
        : "";
      ...
      if (totalGuardadas > 0)
        descriptionParts.push(`Actualizadas ${totalGuardadas} llamadas`);
      if (hadLocks)
        descriptionParts.push(`Bloqueado: ${data.lockedDays.join(", ")}`);
  ...
  const forceButtonLabel = useMemo(() => {
    ...
    if (startKey === endKey) {
      ...
      return startKey === todayKey
        ? "Forzar Hoy"
        : `Forzar ${format(date.from, "dd LLL", { locale: es })}`;
    }
    return "Forzar Rango";
  }, [date]);
  ...
  const totalCalls = useMemo(
    () => performanceData.reduce((s, a) => s + a.totalCalls, 0),
    [performanceData]
  );
  const totalEffectiveCalls = useMemo(
    () => performanceData.reduce((s, a) => s + a.effectiveCalls, 0),
    [performanceData]
  );
  const averageEffectiveness =
    totalCalls > 0 ? (totalEffectiveCalls / totalCalls) * 100 : 0;
  ...
  const handleDateAgent = (preset: string) => {
    if (asesoresOcultos.includes(preset)) {
      const filtered = asesoresOcultos.filter((aO) => aO !== preset);
      localStorage.setItem("asesoresOcultos", JSON.stringify(filtered));
      setAsesoresOcultos(filtered);
    } else {
      const filtered = [...asesoresOcultos, preset];
      localStorage.setItem("asesoresOcultos", JSON.stringify(filtered));
      setAsesoresOcultos(filtered);
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Métricas de llamadas con datos en tiempo real desde Firestore
            </p>
          </div>
        </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center min-h-[400px]">
          ...
          <p className="ml-4 text-muted-foreground">Consultando Firestore...</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  {showDailyBreakdown
                    ? "Haz clic en una fila para ver el desglose por día."
                    : "Selecciona un rango de más de un día para ver desglose."}
                </CardDescription>
              </div>
                  ...
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-center">
                        ...
                        >
                          Intentos {renderSortArrow("totalCalls")}
                        </Button>
                      </TableHead>
                      <TableHead className="text-center">
                        <Button
                        ...
                        >
                          Cumplimiento {renderSortArrow("compliance")}
                        </Button>
                      </TableHead>
                      <TableHead className="text-center">
                        <Button
                          variant="ghost"
                          onClick={() => handleSort("effectiveCalls")}
                        >
                          Efectivas {renderSortArrow("effectiveCalls")}
                        </Button>
                      </TableHead>
                      <TableHead className="text-center">
                        <Button
                        ...
                        >
                          Efectividad {renderSortArrow("effectivenessRate")}
                        </Button>
                      </TableHead>
                      <TableHead className="text-center">
                        <Button
                        ...
                        >
                          Minutos Totales {renderSortArrow("totalSeconds")}
                        </Button>
                      </TableHead>
                      <TableHead className="text-center">
                        <Button
                          variant="ghost"
                          onClick={() => handleSort("firstCallTime")}
                        >
                          Primera Llamada {renderSortArrow("firstCallTime")}
                        </Button>
                      </TableHead>
                      <TableHead className="text-right">
                        ...
                        >
                          Última Llamada {renderSortArrow("lastCallTime")}
                        </Button>
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  {sortedPerformanceData
                    .filter((agnt) => !asesoresOcultos.includes(agnt.id))
                    ?.map((agent) => (
                      <Collapsible
                        asChild
                        key={agent.id}
                        open={openAdvisorId === agent.id}
                        onOpenChange={() =>
                          showDailyBreakdown &&
                          setOpenAdvisorId((p) =>
                            p === agent.id ? null : agent.id
                          )
                        }
                      >
                        <TableBody>
                            ...
                            >
                                ...
                                <TableCell className="p-2">
                                  <ChevronDown
                                    className={cn(
                                      ...
                                      openAdvisorId === agent.id && "rotate-180"
                                    )}
                                  />
                                </TableCell>
                              )}
                              ...
                              <TableCell className="text-center">
                                <Badge
                                  ...
                                  className={cn(
                                    "text-base font-bold",
                                    getComplianceColor(agent.compliance)
                                  )}
                                >
                                  {agent.compliance?.toFixed(0) ?? "N/A"}%
                                </Badge>
                              </TableCell>
                          ...
                          {showDailyBreakdown && (
                            <CollapsibleContent asChild>
                              <TableRow>
                                <TableCell
                                  colSpan={showDailyBreakdown ? 9 : 8}
                                  className="p-0"
                                >
                                  <div className="p-4 bg-muted/50">
                                    <h4 className="font-bold mb-2">
                                      Desglose Diario para {agent.name}
                                    </h4>
                                          ...
                                          {Object.entries(
                                            ...
                                            .map(([d, stats]) => (
                                              <TableRow key={d}>
                                                ...
                                                <TableCell className="text-center font-mono">
                                                  {stats.effectivenessRate?.toFixed(
                                                    1
                                                  ) || "0.0"}
                                                  %
                                                </TableCell>
                                                ...
                                                <TableCell className="text-center font-mono text-green-600">
                                                  {stats.firstCallTime || "N/A"}
                                                </TableCell>
                                                <TableCell className="text-right font-mono text-red-500">
                                                  {stats.lastCallTime || "N/A"}
                                                </TableCell>
                                              </TableRow>
                                            ))}
                                        </TableBody>
